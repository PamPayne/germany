---
source_url: https://dbnsa.sharepoint.com/sites/ComplianceOfficers93/Regulatory Repository/Rest of World/Germany/2. Analysis/20230510_LUGAS_Technische Richtlinie_Zentraldateien V1.11.3 en-GB - ISMS.pdf
country: Germany
...
source_url: https://dbnsa.sharepoint.com/sites/ComplianceOfficers93/Regulatory%20Repository/Rest%20of%20World/Germany/2.%20Analysis/20230510_LUGAS_Technische%20Richtlinie_Zentraldateien%20V1.11.3%20en-GB%20-%20ISMS.pdf
extracted_date: 2026-04-30
jurisdiction: Germany
description: Technical guideline describing the LUGAS central files, interfaces, client certificate management, and player-related operations for multi-state gaming supervision.
...
- Add the possible response for status code 202 (Accepted) to the operation Set player AK-TIV/INACTIVE.
...
**Provider**  
A provider is an organiser of sports betting, online casino games, online poker or virtual slot machine games on the internet who has obtained a permit pursuant to Section 4 GlüStV 2021.

**Limit**  
Cross-provider deposit limit to limit a player's monthly deposits.

**PlayerId**  
Unique identifier of a player at a provider. Is assigned by the provider and must be entered during registration. Not a cross-provider identifier.
...
1 openssl req -new -key client.key -out client.csr -subj "/"
...
- 201 (Created)  
  ...
  - Example (limit increase):
    ...
    1 {  
    2 "currentLimit": 150.00,  
    3 "newLimit": 199.47,  
    4 "newLimitValidAt": "2020-11-20T09:42:29.737Z"  
    5 }

  - Example (limit reduction or limit is maintained):

    1 {  
    2 "currentLimit": 199.47  
    3 }
...
1 {  
2 "limitAmount": 199.26  
3 }

1 PUT /players/{playerId}/Limit
...
1 POST /players/{playerId}/deposits
...
1 GET /players/findByPersonalData
...
1 "x-personalPlayerData: givenName,Max,lastName,Mustermann,postcode 06128,place,Halle (Saale),street,Kaiserslautererstrasse,number,75, countryAlpha2Code,DE,area,DE-ST,birthDate,1990-11-20,birthPlace,Mü nster,birthName,-"
...
The initial issuance of a client certificate is done with the to- ken issued by the authority. The call is made via the public API

Notes:

- The token is valid only once and for 30 days. If an error occurs during the request, the token expires.

1 POST /certificates?token={token}
...
1 curl  
2 --location https://public.api.{domain}/certificates?token={token}  
3 -H "Content-Type: application/pkcs10"  
4 -H "Accept: application/x-x509-user-cert"  
5 --data-binary @client.csr  
6 --output client.pem
...
1 curl  
2 --location https://api.{domain}/certificates  
3 -H "Content-Type: application/pkcs10"  
4 -H "Accept: application/x-x509-user-cert"  
5 --key old_client.key  
6 --cert old_client.pem  
7 --data-binary @new_client.csr  
8 --output new_client.pem
...
1 PUT /certificates/{serial}/revocation
